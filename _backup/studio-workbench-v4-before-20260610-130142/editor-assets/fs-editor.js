(function(){
  ...
  ready(function(){
    ...
    editor.querySelectorAll(".fsx-rail,.fsx-side,.fsx-panel").forEach(el => el.remove());
    ...
    const layoutKey = "flowsync_workbench_v3_" + location.pathname;
    ...
    rail.className = "fsx-rail";
    rail.innerHTML = `
      <div class="fsx-rail-brand">F</div>
      ...
      <button type="button" data-tab="move" class="active" title="Move"><span class="ico">↕</span><span class="txt">Move</span></button>
      <button type="button" data-tab="text" title="Text"><span class="ico">T</span><span class="txt">Text</span></button>
      <button type="button" data-tab="qr" title="QR"><span class="ico">▣</span><span class="txt">QR</span></button>
      <button type="button" data-tab="finish" title="Export"><span class="ico">⇩</span><span class="txt">Save</span></button>
      <div class="fsx-rail-spacer"></div>
      <button type="button" id="fsxRailTheme" title="Theme"><span class="ico">☾</span><span class="txt">Theme</span></button>
    `;
    editor.appendChild(rail);

    const side = document.createElement("div");
    side.className = "fsx-side";
    side.innerHTML = `
      <div class="fsx-side-head">
        <b>Studio</b>
        <small>Pick a layer, then use the top toolbar for fast work or this panel for exact control.</small>
        <div class="fsx-selected-pill"><span>Selected</span><strong id="fsxSelectedName">QR</strong></div>
      </div>

      <div class="fsx-body">
        <section class="fsx-section active" data-section="move">
          <div class="fsx-card">
            <div class="fsx-title"><b>Layer</b><span>select item</span></div>
            <label>Current layer
          ...
          <div class="fsx-card">
            <div class="fsx-title"><b>Position</b><span>exact adjust</span></div>
            <div class="fsx-grid2">
        ...
        <section class="fsx-section" data-section="finish">
          <div class="fsx-card">
            <div class="fsx-title"><b>Export</b><span>always local</span></div>
            <div class="fsx-grid2">
          ...
          <p class="fsx-help">Download and upload are also in the top toolbar for quick reach.</p>
        </section>
      </div>
    `;
    ...
    workbar.innerHTML = `
      ...
      <button type="button" data-tab="finish">Export</button>
      <span class="fsx-divider"></span>

      <label class="fsx-tool-label">Size <input id="fsxTopSize" type="range" min="8" max="92" value="30"></label>
      <label class="fsx-tool-label">Color <input id="fsxTopColor" type="color" value="#101423"></label>
    ...
    function applyTheme(theme){
      ...
      if($("fsxTopTheme")) $("fsxTopTheme").textContent = icon;
      if($("fsxRailTheme")) $("fsxRailTheme").querySelector(".ico").textContent = icon;
    }
    ...
    $("fsxRailTheme").addEventListener("click", toggleTheme);

    function switchTab(tab){
      document.querySelectorAll("[data-tab]").forEach(btn => btn.classList.toggle("active", btn.dataset.tab === tab));
      side.querySelectorAll("[data-section]").forEach(sec => sec.classList.toggle("active", sec.dataset.section === tab));
    }
    ...
    function syncControls(){
      ...
      $("fsxTopSize").value = $("fsxSize").value;
      $("fsxColor").value = rgbToHex(cs.color);
      $("fsxTopColor").value = $("fsxColor").value;
    ...
    side.querySelectorAll("[data-nudge]").forEach(btn => {
    ...
    side.querySelectorAll("[data-align]").forEach(btn => {
      btn.addEventListener("click", () => {
