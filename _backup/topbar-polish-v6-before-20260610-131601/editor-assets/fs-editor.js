(function(){
  ...
  ready(function(){
    ...
    function applyTheme(theme){
      ...
      const railTheme = $("fsxRailTheme");
      if(topTheme) topTheme.textContent = icon;
      if(railTheme) railTheme.querySelector(".ico").textContent = icon;
    }
    ...
    function renderToolbar(){
      ...
      if(isQr){
        workbar.innerHTML = `
          ...
          <label class="fsx-tool-label">Box <input id="fsxTopQrSize" type="range" min="60" max="250"></label>
          <label class="fsx-tool-label">Corner <input id="fsxTopRadius" type="range" min="0" max="90"></label>
          <label class="fsx-tool-label">Scale <input id="fsxTopQrScale" type="range" min="45" max="95"></label>
        ...
        workbar.innerHTML = `
          <select id="fsxTopLayer">${layers.map(([id,name]) => `<option value="${id}">${name}</option>`).join("")}</select>
          <label class="fsx-tool-label">Font
            <select id="fsxTopFont"><option value="Inter">Inter</option><option value="Arial">Arial</option><option value="Georgia">Georgia</option><option value="Trebuchet MS">Trebuchet</option><option value="Impact">Impact</option></select>
          </label>
          <label class="fsx-tool-label">Size <input id="fsxTopSize" type="range" min="8" max="92"></label>
          <label class="fsx-tool-label">Color <input id="fsxTopColor" type="color"></label>
          <button type="button" id="fsxTopCenter">Center</button>
          <button type="button" id="fsxTopFront">Front</button>
          <span class="fsx-toolbar-spacer"></span>
          <button type="button" class="primary" id="fsxTopDownload">Download PNG</button>
